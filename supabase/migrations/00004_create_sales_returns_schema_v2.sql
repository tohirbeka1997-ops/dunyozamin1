...
3. Security
  ...
  - Users can update their own returns or admins can update any
...
CREATE TABLE IF NOT EXISTS sales_returns (
  ...
  CONSTRAINT check_return_status CHECK (status IN ('Pending', 'Completed', 'Cancelled'))
);
...
-- Add constraint for return_status if not exists
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'check_order_return_status'
  ) THEN
    ALTER TABLE orders
    ADD CONSTRAINT check_order_return_status 
