...
export default function CustomerCreditOrdersSheet({
  ...
  const handleDueDateChange = async (row: OpenCreditOrderRow, dueDate: string) => {
    if (!dueDate) return;
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            {rows.length > 0 && (
              <Button
                ...
                onClick={() => void handleSendCreditReminder()}
              >
                {sendingReminder ? (
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {rows.map((row) => {
                ...
                const dueValue = row.due_date ? String(row.due_date).slice(0, 10) : '';
                return (
                  <div
                    key={row.id}
                    ...
                    <div className="flex flex-wrap items-end gap-2">
                      <div className="flex-1 min-w-[10rem] space-y-1">
                        ...
                        <Input
                          ...
                          type="date"
                          value={dueValue}
                          disabled={savingDueDateId === row.id}
                          onChange={(e) => void handleDueDateChange(row, e.target.value)}
                        />
                      </div>
                      {overdue && (
                      ...
                      {row.credit_reminder_note ? (
                        <p className="w-full text-xs text-muted-foreground italic">
                          {t('customers.credit_reminder_note')}: {row.credit_reminder_note}
                        </p>
                      ) : null}
