-- Exclude cancelled / refunded web orders from unified sales (Payme capture
-- then cancel must not stay in P&L). Same discount allocation as 112.
...
SELECT
  ...
  wo.updated_at
FROM web_orders wo
WHERE LOWER(TRIM(COALESCE(wo.status, ''))) NOT IN ('cancelled')
  AND LOWER(TRIM(COALESCE(wo.payment_status, ''))) NOT IN ('refunded', 'failed')
  AND (
    LOWER(TRIM(COALESCE(wo.status, ''))) = 'delivered'
    OR LOWER(TRIM(COALESCE(wo.payment_status, ''))) = 'paid'
  );
